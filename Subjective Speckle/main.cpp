    ...
    void defineSurface(){
        for (int x{0}; x < 5001; x++){
            for (int y{0}; y < 5001; y++){
                surfaceArray[x][y] = tan(((rand() % 90)) / 180.0 * pi) * surfaceRoughness;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            defineAmplitude(z_length + z_length2 + extraLength + lensRefractiveIndex * lensThickness);

//            cout << x_Coordinate2 << " " << y_Coordinate2 << endl;


            if(x_Coordinate2 > -0.1 && x_Coordinate2 < 0.1){
                a = (int)((x_Coordinate2 + 0.1)*1000);
            } else {
                x_Coordinate2 = -0.1;
                a = (int)((x_Coordinate2 + 0.1)*1000);
            }
...
//            minimum = min(minimum, y_Coordinate2);
            ...
            intensityMap[a][b] = amplitude + intensityMap[a][b];
    ...
    void defineintensityMap(){
        ...
        for (int x{0}; x < 201; x++ ){
            for (int y{0}; y < 201; y++){
                ...
                intensityMap [x][y] = defineIntensity(intensityMap[x][y]);
...
// Light variables and Functions
    ...
    double defineAmplitude(double x){ // x is z_length
        amplitude = cos(x /wavelength*2.0*pi);
        return amplitude;
    }

    double defineIntensity(double x){// x is amplitude
        intensity = pow(x, 2);
        return intensity;
...
// Lens and GeomOptics variables and Functions
    double lensFromSurface {0.3}; // distance of screen from surface (Zo + Zr)
    ...
    double lensXCoordinate{0};
    double lensYCoordinate{0};
...
// Coordinate system (Cartesian)
    double x_Coordinate{};
    double y_Coordinate{};
    double arraySpacing {pow(10,-6)}; // each mirror is a square, this refers to the length of the side of the square
...
int main()
{
    ...
    so.defineSurface();
    so.SubjectiveSpeckle();
    so.defineintensityMap();

    ofstream myfile ("Plot2.csv");

    for (int x{0}; x < 201; x++){
        for (int y{0}; y < 201; y++ ){
            myfile << "{" << x << "," << y << "," << intensityMap[x][y] * 1000 << "},";
        }
    }
